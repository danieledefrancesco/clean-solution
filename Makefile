...
CI_PROD_DOCKER_COMPOSE_COMMAND=WEB_PROD_IMAGE_NAME=$(WEB_PROD_IMAGE_NAME) docker-compose -f ${MONGODB_TEST_DOCKER_COMPOSE_FILE} -f ${WEB_PROD_CI_DOCKER_COMPOSE_FILE} -f ${ELK_DOCKER_COMPOSE_FILE} -f ${KARATE_DOCKER_COMPOSE_FILE}
...
run_behavioral_tests_ci:
	...
	$(CI_DEV_DOCKER_COMPOSE_COMMAND) exec -T web bash -c "sudo sh ./scripts/end_coverlet.sh"
