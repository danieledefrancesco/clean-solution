...
start_sonar_scan_ci:	
	...
	$(CI_DEV_DOCKER_COMPOSE_COMMAND) exec web bash ./scripts/wait_sonar_is_green.sh
	$(CI_DEV_DOCKER_COMPOSE_COMMAND) exec web bash ./scripts/create_sonar_project.sh
	$(CI_DEV_DOCKER_COMPOSE_COMMAND) exec web bash ./scripts/begin_sonar_scan.sh
	
end_sonar_scan_ci:
	$(CI_DEV_DOCKER_COMPOSE_COMMAND) exec web bash ./scripts/end_sonar_scan.sh
...
run_behavioral_tests_ci:
	$(CI_DEV_DOCKER_COMPOSE_COMMAND) exec -d web bash -c "sh ./scripts/start_with_coverlet_watch.sh > coverlet.log"
	$(CI_DEV_DOCKER_COMPOSE_COMMAND) run karate make test
	$(CI_DEV_DOCKER_COMPOSE_COMMAND) exec web bash ./scripts/end_coverlet.sh
